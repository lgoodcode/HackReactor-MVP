...
export default function Menu({ isDesktop }: MenuProps) {
  return (
    <Popover className="user relative">
      {isDesktop ? (
        ...
        <div className="h-full centered">
          <Popover.Button className="w-[42px] h-[42px] centered rounded-full cursor-pointer outline-none">
            <Hamburger w={48} h={48} />
          </Popover.Button>
        </div>
      )}
      ...
      >
        <Popover.Panel className="absolute w-[192px] -left-8 z-10 mt-3 -translate-x-1/2 transform px-4 sm:px-0">
